...
class visual_prompt(nn.Module):
    def __init__(self, sim_head, clip_state_dict, T):
        ...
        if self.sim_header == "LSTM" or self.sim_header == "Transf" or self.sim_header == "Transf_cls" or self.sim_header == "Conv_1D" :
            ...
            self.frame_position_embeddings = nn.Embedding(context_length, 768)
...
class Adapter(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = x.float()
        x_fc1 = self.fc1(x2) #### 粗心 qiao ，根本没调用原先，导致梯度为0 ####
        x_fc2 = self.act(x_fc1)
        x_fc3 = self.fc2(x_fc2)
        image_features = self.ratio * x_fc3 + (1 - self.ratio) * x2
...
class clsHead(nn.Module):
    def __init__(self, c_in=768,  numcls=51,ratio=0.2):
        ...
        #     # nn.ReLU(inplace=True)
        # )
        self.fc1 = nn.Linear(c_in,numcls)
        self.apply(self._init_weights)
    ...
    def forward(self, x):
        ...
        x_fc1 = self.fc1(x) #### 粗心 qiao ，根本没调用原先，导致梯度为0 ####
